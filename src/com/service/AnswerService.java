...
public class AnswerService {
    // 假设您已经定义了序列：answer_sheets_seq 和 answer_details_seq
    ...
    public void saveAnswer(int sheetId, int questionId, String userAnswer) throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // 新增答案
                String getSeqSql = "SELECT answer_details_seq.NEXTVAL FROM DUAL"; // ⚠️ 请替换为实际序列名
    ...
    // 提交试卷（自动判分客观题）
    public void submitPaper(int sheetId) throws SQLException {
        Connection conn = DBUtil.getConnection();
        ...
        try {
            // 1. 更新答卷状态
            String updateSheetSql = "UPDATE answer_sheets SET submit_time = ?, status = 'SUBMITTED' WHERE sheet_id = ?";
            sheetPstmt = conn.prepareStatement(updateSheetSql);
            sheetPstmt.setTimestamp(1, new java.sql.Timestamp(new Date().getTime()));
            sheetPstmt.setInt(2, sheetId);
            ...
            while (rs.next()) {
                ...
                int score = rs.getInt("score"); // 题目的满分
                int detailId = rs.getInt("detail_id");

                // 添加分值检查
                if (score == 0) {
                    System.out.println("警告：题目ID " + questionId + " 在查询结果中分值为0！");
                }
                ...
                if ("SINGLE".equals(type) || "MULTIPLE".equals(type) || "FILL".equals(type)) {
                    ...
                    // 输出调试日志
                    System.out.println("题目ID: " + rs.getInt("question_id") +
                            ", 题型: " + type +
                            ", 正确答案: " + correctAnswer +
                            ", 用户答案: " + userAnswer +
                    ...
                    System.out.println("当前累计客观分: " + objectiveScore);
                }
                // 主观题跳过
            }
            ...
            scorePstmt.executeUpdate(); // 【重要】：确保执行更新

            conn.commit();

            System.out.println("最终客观题总分: " + objectiveScore); // 添加最终分数日志
    ...
    public double getObjectiveScore(int sheetId) { // 修改返回类型为 double 兼容数据库
        double score = 0;
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
